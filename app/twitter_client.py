...
class TwitterClient:
    ...
    def __init__(self, settings: Settings):
        """
        Initialize Twitter client with settings.
                ...
                if instruction.get("type") == "TimelineAddEntries":
                    ...
                    for entry in entries:
                        ...
                        if entry_id.startswith("cursor-bottom"):
                            next_cursor = self._extract_cursor(entry)
    ...
    def _extract_cursor(self, entry: Dict[str, Any]) -> Optional[str]:
    ...
    def _extract_tweet_from_entry(self, entry: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Extract tweet data from a tweet entry."""
        content = entry.get("content", {})
        ...
        if result.get("__typename") == "Tweet":
            return self._extract_tweet_data(result)
        return None

    def _extract_tweet_data(self, tweet_result: Dict[str, Any]) -> Dict[str, Any]:
        """
        Extract relevant data from a tweet result object.

        Args:
            tweet_result: Tweet result object from API
        ...
        has_image, has_video = self._extract_media_flags(legacy)

        # Build tweet URL
        url = self._build_tweet_url(author_username, tweet_id)

        # Get tweet text (handle long tweets)
        text = self._extract_tweet_text(tweet_result, legacy)
    ...
    def _extract_media_flags(self, legacy: Dict[str, Any]) -> Tuple[bool, bool]:
        """
        Extract media flags from tweet legacy data.

        Args:
            legacy: Legacy data from tweet

        Returns:
            Tuple of (has_image, has_video)
        """
    ...
    def _build_tweet_url(self, username: str, tweet_id: str) -> Optional[str]:
        """Build tweet URL from username and tweet ID."""
        if username and tweet_id:
            return f"https://x.com/{username}/status/{tweet_id}"
        return None

    def _extract_tweet_text(
        self, tweet_result: Dict[str, Any], legacy: Dict[str, Any]
    ) -> str:
        """
        Extract tweet text, handling long tweets (note tweets).

        Args:
            tweet_result: Full tweet result object
            legacy: Legacy data from tweet
        ...
        # Check for long-form note tweet
